...
//start1 根据 coalesced 原理，subB 矩阵 8x128 的加载改成了 interleave32，thread_i 要取 i, i+32, i+64, i+96 这四个数据。
  ...
  for (int loop = 0; loop < k; loop += 8) {
    ...
    auto share2reg = [&](int index,float* ptrA,float* ptrB){
      for (int i = 0; i < 4; ++i) {
        panelA[index][i] = ptrA[i];
        panelA[index][i + 4] = ptrA[i + 64];
        
      }
      for (int i = 0; i < 4; ++i) {
        panelB[index][i] = ptrB[i];
        panelB[index][i + 4] = ptrB[i + 64];
      }
    };
    auto calc_subk = [&](int index){
      for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
          sum[i][j] += panelA[index][i] * panelB[index][j];
        }
      }
    };
    float *ptrA = ashare + aidx0 + subk * SMEM_LDA;
    float *ptrB = bshare + bidx0 + subk * SMEM_LDB;
    share2reg(0,ptrA,ptrB);
    for (int subk = 0; subk < 8; ++subk) {
        float *ptrA = ashare + aidx0 + ((subk + 1) % 8) * SMEM_LDA;
        float *ptrB = bshare + bidx0 + ((subk + 1) % 8) * SMEM_LDB;
        share2reg((subk + 1) % 2);
        calc_subk(subk%2);
    }
    __syncthreads();
