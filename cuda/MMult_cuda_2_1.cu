#include <assert.h>
#include <stdlib.h>

// CUDA runtime
#include "helper.h"
#include <cublas_v2.h>
#include <cuda_runtime.h>

/*
naive 版2.0 

https://zhuanlan.zhihu.com/p/342103911

 * 一个线程计算a的一行与b的一列相乘，
 * 计算c(mxn)上坐标为[_m，_n]的结果,1维坐标是[_m*n+_n]，
 * 对应a的[_m，...]一维：[_m * k :_m * k+k ：1]，
 * 与b的[...，_n],一维： [ _n    ：k*n+_n  : n]，
 * 分块计算：
 * 计算 a[_m * k : _m * k+k ：block] *  b[ _n ：k*n+_n  : block]
 * 分块过后便于使用share_mem，因为share_mem大小有限
 * */
template <int BLOCK>
__global__ void sgemm(int m, int n, int k, float *a, int lda, float *b, int ldb,
                      float *c, int ldc)
{
  // blockIdx control subpanel matrix
  int _m = blockIdx.x * blockDim.x + threadIdx.x; //Row of matrix C
  int _n = blockIdx.y * blockDim.y + threadIdx.y; //Column of matrix C
  float *a_ptr = a + _m * k；
  float *b_ptr = b + _n;
  float *end_a = a + (_m * k + k);
  float sum = 0.f;
  for (; a_ptr < end_a;
       a_ptr += BLOCK, b_ptr += BLOCK * n)
  {
    for (int kk = 0; kk < BLOCK; ++kk)
    {
      sum += a_ptr[kk] * b_ptr[kk*n];
    }
  }
  c[_m * n + _n] = sum;
}

void MY_MMult(cublasHandle_t handle, int m, int n, int k, float *d_A, int lda,
              float *d_B, int ldb, float *d_C, int ldc)
{

  constexpr int BLOCK = 16;
  dim3 block(BLOCK, BLOCK);
  dim3 grid((m + BLOCK - 1) / BLOCK, (n + BLOCK - 1) / BLOCK);

  sgemm<BLOCK><<<grid, block>>>(m, n, k, d_A, lda, d_B, ldb, d_C, ldc);
}
